...
export const useBuildNumber = (lettersWanted = 7) => {
  const { data: buildNumberData } = useQuery({
    queryFn: () =>
      axios
        .get(
          `https://api.github.com/repos/validblocks/xSafe/commits?sha=${relatedBranch}&per-page=1`,
          {
            headers: {
              Authorization: `Token ${import.meta.env.VITE_GH_ACCESS_TOKEN}`,
            },
          },
        )
        .then((r) => r.data),
    enabled: !!import.meta.env.VITE_GH_ACCESS_TOKEN,
