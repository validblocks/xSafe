...
import { useIsAlreadyProposedMap } from 'src/hooks/useIsAlreadyProposedMap';
...
function NftCard({ nft }: Props) {
  ...
  const sendButtonText = useMemo(
    () => (isSFT ? 'Send SFT' : 'Send NFT'),
    [isSFT],
  );

  const handleOptionSelected = useCallback(
    (option: ProposalsTypes, nft: NFTType) => {
      dispatch(setProposeMultiselectSelectedOption({ option }));
      dispatch(setSelectedNftToSend(nft));
    },
    [dispatch],
  );

  const handleSendNftClick = useCallback(
    (nft: NFTType) =>
      (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        handleOptionSelected(
          isSFT ? ProposalsTypes.send_sft : ProposalsTypes.send_nft,
          nft,
        );
  ...
  useEffect(() => {
    if (typographyRef.current) {
      setAdjustedText(
        adjustTextByWidth({
          text: nft.name,
          containerWidth: typographyRef.current?.offsetWidth ?? 0,
  ...
  return (
    <Styled.NftCardBox>
      {isAlreadyProposed[nft.identifier] && <PendingNftProposalAnnouncer />}
      ...
      >
        <Styled.NftCardMedia
          sx={{
            borderBottomLeftRadius: hasNftDetailsEnabled
              ? '0'
              : '4px !important',
            borderBottomRightRadius: hasNftDetailsEnabled
              ? '0'
              : '4px !important',
          }}
