...
import { MultisigRemoveUser } from 'src/types/multisig/proposals/MultisigRemoveUser';
...
function ProposeRemoveUser({
  ...
  const removeUserProposals = useMemo(() => {
    ...
    return (
      cachedPendingActions?.filter(
        (p: MultisigActionDetailed) => p.action instanceof MultisigRemoveUser,
      ) ?? []
    );
  }, [queryClient]);
  ...
  return (
    <div className="modal-control-container">
      ...
      <label htmlFor="delegateSubTitle">
        <Text>{t('Address') as string}</Text>
      </label>
      <div
        id="delegateSubTitle"
