...
const AppCard = ({
  ...
  return (
    <Styled.AppCard>
      <img className="rounded w-100" src={imgUrl} alt="member" />
      <Text fontSize={20} fontWeight={500} margin="12px 0px 6px">
        {title}
      </Text>
      <Text fontSize={15} fontWeight={400} marginBottom={'12px'}>
        {description}
      </Text>
      <Box
        display={'flex'}
      ...
      >
        <Styled.InstallButton
          ...
          sx={{
            marginRight:
              isInstallable && isInstalled && maxWidth600 ? '12px' : '0',
          }}
          disabled={
            !isInstallable || isInReadOnlyMode || title === 'Marketplace'
          }
        >
          {isInstallable ? actionButtonText : 'Coming Soon...'}
        </Styled.InstallButton>
        {isInstallable && maxWidth600 && isInstalled ? (
          <Styled.PinButton onClick={actionButtonOnPin}>
